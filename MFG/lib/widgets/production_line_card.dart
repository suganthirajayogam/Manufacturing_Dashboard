...
class FinalCompactCard1 extends StatefulWidget {
  ...
  @override
  State<FinalCompactCard1> createState() => _FinalCompactCardState();
}

class _FinalCompactCardState extends State<FinalCompactCard1>
  ...
  double _getResponsiveFontSize(double baseFontSize) {
    ...
    // Special handling for 5 cards per row
    if (cardsPerRow == 5) {
      // For 5 cards, ensure fonts remain readable despite small scale
      if (baseFontSize >= 40) { // Large numbers (target/actual)
        return baseFontSize * 0.85; // Reduce slightly but keep readable
      } else if (baseFontSize >= 18) { // Medium text (titles)
        return baseFontSize * 0.9; // Small reduction
      } else if (baseFontSize >= 14) { // Regular text
        return baseFontSize * 0.95; // Minimal reduction
      } else { // Small text
        return baseFontSize; // Keep original size for readability
      }
    }
    
    // Original scaling logic for other card counts
    if (scale >= 2.5) {
      ...
      // For very small scales (like 5 cards), ensure minimum readability
  ...
  double _getResponsivePadding(double basePadding) {
    ...
    // Special handling for 5 cards per row - reduce padding more aggressively
    if (cardsPerRow == 5) {
      return basePadding * 0.6; // Significantly reduce padding
    }
  ...
  double _getResponsiveSpacing(double baseSpacing) {
    ...
    // Special handling for 5 cards per row - reduce spacing more
    if (cardsPerRow == 5) {
      return baseSpacing * 0.5; // Significantly reduce spacing
    }
  ...
  void initState() {
    ...
    // New animation controller for status pulse
    _statusPulseController = AnimationController(
      duration: const Duration(milliseconds: 1500),
      vsync: this,
    );
    
    // New animation controller for smooth progress transitions
  ...
  @override
  void didUpdateWidget(FinalCompactCard1 oldWidget) {
    ...
    // Trigger progress animation on data change
    if (widget.data != null && oldWidget.data != null) {
      final newPercentage = widget.data!.hrXhrData.target > 0
          ? (widget.data!.hrXhrData.totalProduction / widget.data!.hrXhrData.target * 100)
          : 0.0;
      final oldPercentage = oldWidget.data!.hrXhrData.target > 0
          ? (oldWidget.data!.hrXhrData.totalProduction / oldWidget.data!.hrXhrData.target * 100)
          : 0.0;
  ...
  Map<String, int> _getCurrentShiftDetails() {
    ...
    List<int> intervals = (shifts[currentShiftName]!['intervals'] as List<dynamic>).cast<int>();
    ...
    shiftStartMinutes = shifts[currentShiftName]!['start'] as int;
    intervals = (shifts[currentShiftName]!['intervals'] as List<dynamic>).cast<int>();
    ...
    int currentShiftHour = (currentIntervalIndex == -1) ? totalHours : currentIntervalIndex + 1;
  ...
  Widget build(BuildContext context) {
    // Return loading state if animations are not initialized yet
    if (!_animationsInitialized || _scaleAnimation == null) {
      return _buildLoadingCard();
    }
    
    final scale = widget.settings.cardScale;
    final shiftDetails = _getCurrentShiftDetails();
          ...
          child: AnimatedOpacity(
            ...
            child: Container(
              decoration: BoxDecoration(
                color: widget.settings.darkMode ? const Color(0xFF2E4057) : Colors.white,
              ...
              child: Stack(
                children: [
                  if (widget.data == null)
                    ...
                    _buildDataCard(currentShiftHour, totalShiftHours),
                  if (widget.isStale && widget.settings.showStaleDataWarning && widget.data != null && _warningAnimation != null)
                    AnimatedBuilder(
          ...
          children: [
            ...
            Text(
              ...
              style: TextStyle(
                ...
                color: widget.settings.darkMode ? Colors.white : Colors.black87,
              ),
            ),
          ],
        ),
  ...
  Widget _buildDataCard(int currentShiftHour, int totalShiftHours) {
    ...
    try {
      if (andonStatus.initial != null && andonStatus.initial.isNotEmpty) {
        stopTime = DateTime.parse(andonStatus.initial);
      ...
      child: LayoutBuilder(
        builder: (context, constraints) {
          return Column(
            ...
            children: [
              // Header row with line name and status
              Row(
                ...
                children: [
                  ...
                  Flexible(
                    child: (_statusPulseAnimation != null && _glowAnimation != null) 
                      ? AnimatedBuilder(
                          animation: Listenable.merge([_statusPulseAnimation!, _glowAnimation!]),
                          builder: (context, child) {
                            return Transform.scale(
                              ...
                              child: Container(
                                ...
                                decoration: BoxDecoration(
                                  ...
                                  border: Border.all(color: statusColor, width: _getResponsiveSize(1.5)),
                                  boxShadow: [
                                    // Glow effect
                                    BoxShadow(
                                      color: statusColor.withOpacity(_glowAnimation!.value * 0.6),
                                      blurRadius: _getResponsiveSize(12),
                                      spreadRadius: _getResponsiveSize(2),
                                    ),
                                  ],
                                ),
                                child: Row(
                                  ...
                                  children: [
                                    Icon(
                                      _getStatusIcon(andonStatus.status),
                                      color: statusColor,
                                      size: _getResponsiveSize(18),
                                    ),
                                    SizedBox(width: _getResponsiveSpacing(3)),
                                    Flexible(
                                      child: Text(
              ...
              // Target and Actual metrics with smooth animations
              IntrinsicHeight(
                child: Row(
                  children: [
                    _buildMetricItem(
                      'Target',
                      productionData.target.toString(),
                      Colors.white,
                      Icons.flag,
                    ),
              ...
              AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  ...
                  borderRadius: BorderRadius.circular(_getResponsiveSize(6)),
                ),
                child: Row(
                  children: [
                    Expanded(
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          color: widget.settings.darkMode
                              ? Colors.white10
                              : Colors.black.withOpacity(0.05),
                          borderRadius: BorderRadius.circular(_getResponsiveSize(3)),
                        ),
                        child: Text(
                          lineOverview.modelType,
                          style: TextStyle(
                            color: widget.settings.darkMode ? Colors.white : Colors.black87,
              ...
              // Rate section with enhanced animations
              Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Row(
                    ...
                    children: [
                      Text(
                        'Rate',
                        style: TextStyle(
                          color: widget.settings.darkMode ? Colors.white70 : Colors.black54,
                      ...
                      (_progressAnimation != null) 
                        ? AnimatedBuilder(
                            ...
                            builder: (context, child) {
                              final animatedPercentage = _previousPercentage + 
                                  (percentage - _previousPercentage) * _progressAnimation!.value;
                              return Text(
                                '${animatedPercentage.toStringAsFixed(1)}%',
                                style: TextStyle(
                                  color: _getProductionColor(animatedPercentage),
                                  fontWeight: FontWeight.bold,
                                  fontSize: _getResponsiveFontSize(20),
                                  shadows: [
                                    Shadow(
                                      color: _getProductionColor(animatedPercentage).withOpacity(0.5),
                                      blurRadius: 8,
                                    ),
                                  ],
                                ),
                  ...
                  SizedBox(height: _getResponsiveSpacing(3)),
                  ClipRRect(
                    borderRadius: BorderRadius.circular(_getResponsiveSize(4)),
                    child: (_progressAnimation != null && _glowAnimation != null)
                      ? AnimatedBuilder(
                          animation: _progressAnimation!,
                          builder: (context, child) {
                            final animatedValue = (_previousPercentage / 100) + 
                                ((percentage - _previousPercentage) / 100) * _progressAnimation!.value;
                            return Stack(
                              children: [
                                LinearProgressIndicator(
                                  value: animatedValue,
                                  backgroundColor: widget.settings.darkMode ? Colors.white12 : Colors.grey[200],
                                  valueColor: AlwaysStoppedAnimation<Color>(_getProductionColor(animatedValue * 100)),
                                  minHeight: _getResponsiveSize(8), // Increased height for better visibility
                                ),
                      ...
                      : LinearProgressIndicator(
                          ...
                          backgroundColor: widget.settings.darkMode ? Colors.white12 : Colors.grey[200],
                          valueColor: AlwaysStoppedAnimation<Color>(_getProductionColor(percentage)),
              ...
              // Hour indicators with animations
              Expanded(
                child: Row(
                  ...
                  children: List.generate(totalShiftHours, (index) {
                    ...
                    return _buildHourIndicator(hour, hourData, isCurrentHour);
                  }),
                ),
              ),
            ],
  ...
  Widget _buildMetricItem(String label, String value, Color color, IconData icon) {
    ...
    // Special sizing for 5 cards per row
    double verticalPadding = cardsPerRow == 5 ? 8.0 : _getResponsivePadding(16);
    double horizontalPadding = cardsPerRow == 5 ? 6.0 : _getResponsivePadding(12);
        ...
        ? AnimatedBuilder(
            ...
            builder: (context, child) {
              ...
              if (label == "Target" && _previousTarget > 0) {
                animatedValue = (_previousTarget + (targetValue - _previousTarget) * _progressAnimation!.value).round();
              } else if (label == "Actual" && _previousActual >= 0) {
                animatedValue = (_previousActual + (targetValue - _previousActual) * _progressAnimation!.value).round();
              }
              ...
              return AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  color: label == "Target" ? Colors.teal.withOpacity(0.2) : color.withOpacity(0.1),
              ...
              children: [
                FittedBox(
                  ...
                  child: Text(
                    "$label(nos)",
                    style: TextStyle(
                      color: widget.settings.darkMode ? Colors.white : Colors.black54,
                      fontSize: labelFontSize,
                      fontWeight: FontWeight.w500,
                    ),
                  ),
  ...
  Widget _buildHourIndicator(int hour, HourlyProduction hourData, bool isCurrentHour) {
    ...
    if (hourData.status == 'not-started') {
      ...
      double achievement = hourData.target > 0 ? (hourData.production / hourData.target * 100) : 100;
      if (achievement >= 90) {
                  ...
                  return Transform.scale(
                    ...
                    child: Container(
                      decoration: BoxDecoration(
                        ...
                        boxShadow: [
                          // Enhanced glow for current hour
                          BoxShadow(
                      ...
                      child: Center(child: FittedBox(fit: BoxFit.scaleDown, child: centerWidget)),
                    ),
                  );
                },
              )
              ...
              return Transform.scale(
                ...
                child: (_glowAnimation != null)
                  ? AnimatedBuilder(
                      animation: _glowAnimation!,
                      builder: (context, child) {
                        // Add subtle glow to completed hours
                        final glowOpacity = isComplete ? _glowAnimation!.value * 0.3 : 0.2;
                        return Container(
                          decoration: BoxDecoration(
                            color: indicatorColor,
                            shape: BoxShape.circle,
                            boxShadow: [
                              BoxShadow(
                                color: indicatorColor.withOpacity(glowOpacity),
                                blurRadius: _getResponsiveSize(8),
  ...
  IconData _getStatusIcon(String status) {
    ...
    if (statusLower.contains('break') || statusLower.contains('others')) return Icons.pause;
    return Icons.info;
  }
  ...
  Color _getStatusColor(String status) {
    ...
    if (statusLower.contains('running')) {
      return Colors.green;
    } else if (statusLower.contains('line feeding delay') || statusLower.contains('maintenance')) {
      return Colors.red;
    } else if (statusLower.contains('break') || statusLower.contains('lunch') || statusLower.contains('others')) {
      return Colors.grey;
    }
